...
import {decode, encode, getLettersFromTree, getTreeFromSentence} from "./services/encoding/entropy/huffman";
import {calculateAverageCodeLength, calculateEntropyForLetters, calculateRedundancy} from "./services/encoding/entropy";
import EncodingDictionary from "./services/encoding/dictionary/dictionaryModel";
...
class App extends Component {
    ...
    handleCalculationFromFile = text => {
        try {
            ...
            let tree = getTreeFromSentence(text);
            let letters = getLettersFromTree(tree);

            let huffmanEncoded = encode(text, letters);
            let huffmanDecoded = decode(huffmanEncoded, tree);

            let redundancy = calculateRedundancy(letters);
            let averageCodeLength = calculateAverageCodeLength(letters);
            let entropy = calculateEntropyForLetters(letters);
    ...
    onLettersInput = (letters, generatedText) => {
        this.setState(() => ({
            letters: letters,
            entropy: calculateEntropyForLetters(letters),
            redundancy: calculateRedundancy(letters),
            averageCodeLength: calculateAverageCodeLength(letters)
        }));
    };

    onHandInput = (letters, text) => {
        try {
            let huffmanEncoded = encode(text, _.cloneDeep(letters));
            let tree = getTreeFromSentence(text);
            let huffmanDecoded = decode(huffmanEncoded, tree);
